{{-- <x-app>
      <div class="container">
            <div class="row">
                  <p class="text-center">Halo {{ $checkout->user->name }}</p>
                  <p class="mb-1">{{ $checkout->product->id }}</p>
                  <p class="mb-1">{{ $checkout->product->name }}</p>
                  <p class="mb-1">{{ $checkout->product->description }}</p>
                  <p class="mb-1">{{ $checkout->product->stock }}</p>
                  <p class="mb-1">{{ $checkout->product->price }}</p>
                  <p class="mb-1">{{ $checkout->size->name }}</p>
                  <p class="mb-1">{{ $checkout->quantity }}</p>
                  <div class="mb-1">
                  ...
                  <form action="{{ route('checkones.delete') }}" method="POST" enctype="application/x-www-form-urlencoded">
                        @csrf
                                                ...
                                                <div class="d-flex gap-2" style="overflow:auto">
                                                      ...
                                                      <div class="col-4">
                                                            <div class="card col">
                                                                  <div class="row g-0">
                                                                        <div class="col-4">
                                                                              <img src="{{ url('/storage/images/products/' . $checkout->product->image) }}" class="rounded float m-3" alt="Image" style="width: 8vw; height: auto; object-fit:cover; border: 0.5vw; border-color: black;">
                                                                        </div>
                                                                        <div class="col-8">
                                                                              <div class="card-body">
                                                                                    <h5 class="fw-semibold">{{ $checkout->product->name }}</h5>
                                                                                    <div class="border" style="background: linear-gradient(to right, #E200F7, #04BBEC); height: 2px;"></div>
                                                                                    <h6 class="my-2">Size: {{ $checkout->size->name }}</h6>
                                                                                    <h6 class="my-2 fw-semibold">Rp {{ number_format($checkout->product->price,0,',','.') }}</h6>
                                                                                    <h6>Quantity: {{ $checkout->quantity }}</h6>
                                                                              </div>
                                                                        </div>
                                                                  </div>
                                                            </div>
                              ...
                              <div class="row">
                                    ...
                                    <div class="card p-4" style="background-color: #2A2A2A; color: white;">
                                          <div class="row g-0">
                                                ...
                                                <h4>Rp {{ number_format($checkout->totalPriceProduct,0,',','.') }}</h4>
                                                </div>
